...
# The script will also create an overlap map of the sulci over a standard surface. Also, to see its overlap with atlases in standard space.
# Examples of atlases are Wang and Yeo 17
# The Yeo is available from the HCP database and can be downloaded from here https://balsa.wustl.edu/study/WG33, and already in dlabel.nii format in 32K resolution
# The Wang HCP 7T Retinotopy dataset, including the Wang 2015 atlas, can be downloaded from https://balsa.wustl.edu/study/9Zkk and is already in dlabel.nii format in 32k resolution
...
# 3.2 Add the dense labels: SULCI, WANG, RSN, MPM, and BORDER FILES
wb_command -add-to-spec-file \
        $WB_PATH/AVG_dscalar.32k_fs_LR.wb.spec \
        CORTEX \
        $WORK/All.SULCI.32k_fs_LR.dlabel.nii
...
wb_command -add-to-spec-file \
        $WB_PATH/AVG_dscalar.32k_fs_LR.wb.spec \
        CORTEX \
        $WB_PATH/Wang/S1200_7T_Retinotopy181/MNINonLinear/fsaverage_LR32k/lr.wang2015.32k_fs_LR.dlabel.nii
wb_command -add-to-spec-file \
        $WB_PATH/AVG_dscalar.32k_fs_LR.wb.spec \
        CORTEX \
        $WB_PATH/RSN/null_lL_WG33/RSN-networks.32k_fs_LR.dlabel.nii
wb_command -add-to-spec-file \
        $WB_PATH/AVG_dscalar.32k_fs_LR.wb.spec \
        CORTEX \
        $WORK/MPM_0.33_new.dlabel.nii
...
# 6. Calculate the overlap fraction between label files
# 6.1. Create a dense scalar ROI file from the dense label
wb_command -cifti-all-labels-to-rois \
    $WORK/MPM_0.33_new.dlabel.nii\
    1 \
    $WORK/MPM_0.33_col.dscalar.nii

# 6.2. Prepare the atlases
# 6.2.1 Separate into labels
for label in YEO WANG; do
    wb_command -cifti-separate \
        $ATLAS/${label}.dlabel.nii \
        COLUMN \
        -label \
        CORTEX_LEFT \
        $ATLAS/lh.${label}.label.gii \
        -label \
        CORTEX_RIGHT \
        $ATLAS/rh.${label}.label.gii 
done
...
# 6.2.3 Create new dense labels
for label in YEO WANG; do
    wb_command -cifti-create-label \
        $ATLAS/${label}_new.dlabel.nii \
        -left-label \
        $ATLAS/lh.${label}.label.gii \
        -roi-left \
        $HCP_S1200/S1200.L.roi_MSMAll.32k_fs_LR.shape.gii \
...
# 6.3 Parcellate the dscalar against the target dense label (Yeo other method see below)

for label in WANG; do
    wb_command -cifti-parcellate \
        $WORK/MPM_0.33_col.dscalar.nii \
        $ATLAS/${label}_new.dlabel.nii \
        COLUMN \
        $ATLAS/$label.pscalar.nii \
        -legacy-mode
done

# 6.4. Export the parcellation results
# 6.4.1. Define the tables
for label in WANG; do
...
# Export the values of the parcel
for label in WANG; do
    wb_command -cifti-convert \
        -to-text \
        $ATLAS/$label.pscalar.nii \
        $WB_PATH/Overlap_Parcel/${label}_parcel.txt
    done

# REVERSE the work for the YEO. The reason is the Yeo parcels are larger than the sulci parcels.
# Create a dense scalar ROI file from the dense label

wb_command -cifti-all-labels-to-rois \
    $WB_PATH/ALL_ATLASES/YEO_new.dlabel.nii \
    2 \
   $WB_PATH/ALL_ATLASES/YEO_17.dscalar.nii

# Paracellate

for scalar in YEO_17; do
    wb_command -cifti-parcellate \
        $ATLAS/$scalar.dscalar.nii \
        $WORK/MPM_0.33_new.dlabel.nii \
        COLUMN \
        $ATLAS/$scalar.sulci.pscalar.nii \
        -legacy-mode
done

# Export the values to texts
for scalar in YEO_17; do
    wb_command -cifti-convert \
        -to-text \
        $ATLAS/$scalar.sulci.pscalar.nii \
        $WB_PATH/Overlap_Parcel/$scalar.sulci.text
done

# 7. Display the MNI coordinates over the brain surface. For each coordinate XYZ triple, find the closest vertex in the surface
# You can use CLI. However, since the number of coordinates was small, we used the wb_view GUI. Refer to the manual of wb_view at https://www.humanconnectome.org/storage/app/media/documentation/tutorials/Connectome_WB_Tutorial_v1.5.pdf